...
.\" (faith@cs.unc.edu)
.\" "
.TH SETSERIAL 8 "27 Sierpieñ 1994" "Setserial 2.10" "Podrêcznik programisty linuxowego"
.SH NAZWA
...
port I/O, numer IRQ, to czy klawisz break powinien byæ interpretowane jako
Secure Attention Key, itd.

Podczas normalnego bootowania, inicjalizowane s± tylko porty COM 1-4;
u¿ywane s± domyslne warto¶ci portów I/O i IRQ. Aby zainicjalizowaæ dodatkowe
porty szeregowe, lub aby zmieniæ konfiguracje portów 1-4, u¿yj programu
.BR setserial .
Zazwyczaj jest on wo³any ze skryptu
.IR rc.serial ,
który jest normalnie uruchomiany z
.IR /etc/rc.local .

Argument(y)
.I device
okre¶laj±, które urzadzenie szeregowe ma byæ skonfigurowane, lub zapytane.
Zazwyczaj przyjmuje postaæ
.BR /dev/cua[0-3] .

Bez parametrów,
.B setserial
wydrukuje rodzaj portu np. 8250, 16450, 16550, 16550A), sprzêtowy port I/O,
...
trzeba wydrukowaæ charakterystykê.
...
interpretowane jako parametry, które powinny byæ za³±czone do tego
urz±dzenia.
...
podsumowanie konfiguracji urz±dzenia, które mo¿e byæ wystarczaj±ca do
...
.TP
.B \-W
Dokonaj dzikiej inicjalizacji przerwañ i zakoñcz.

.SH PARAMETRY
Portowi szeregowemu mo¿na przyznaæ nastêpuj±ce parametry.

Wszystkie warto¶ci argumentów s± warto¶ciami dziêsiêtnymi, chyba ¿e
poprzedzone s± "0x".
...
poniewa¿ oryginalne 16550 maj± b³êdy czyni±ce FIFO bezu¿ytecznym, FIFO mo¿e
byæ u¿ywane tylko na chipach zidentyfikowanych jako UART-y 16550A.
Ustawienie typu UART na 8250, 16450, lub 16550 w³±czy port szeregowy bez
próby u¿ywania FIFO. U¿ycie typu UART
.B none
wy³±czy port.
...
ten parametrm aby mo¿na by³o zainicjalizowaæ UART prawid³owo.
.TP
.B ^skip_test
.I Nie
...
szybko¶ci. W tym wypadku, przêdko¶æ jest okre¶lona przez
.B baud_base
podzielone przez
.BR divisor (dzielnik).
...
.TP
.B session_lockout
Lokuj dostêp do portu wydzwaniaj±cego (/dev/cuaXX) na przestrzeni ró¿nych
...
.TP
.B pgrp_lockout
Lokuj port wydzwaniaj±cy (/dev/cuaXX) na przestrzeni ró¿nych grup procesów.
...
.TP
.B ^pgrp_lockout
Nie lokuj portu wydzwaniaj±cego na przestrzeni ró¿nych grup procesów.
.TP
.B hup_notify
...
Zastosowanie tego parametru odnosi siê do getty, które sa blokowane na
...
Wa¿nym jest, by zauwa¿yæ i¿ setserial zwyczajnie mówi j±dru Linuxa, gdzie
...
gdy spróbujesz u¿ywaæ ich naraz. Ograniczenie to mo¿na obej¶æ prez specjalne
wieloportowe karty szeregowe, które s± skonstruowane do dzielenia wielu
portów szeregowych na jednej linii IRQ. Karty wspierane przez Linuxa
...
Wiele osób uwa¿a, ¿e IRQ 5 jest dobrymw wyborem, zak³adaj±c ¿e w komputerze
...
wykorzystaæ to IRQ dla innej karty. Nie jest to niezbêdne pod GNU/Linuxem i
...
.SH UWAGA
UWAGA: U¿ywanie nieprawid³ego portu mo¿e zalokowaæ twoj± maszynê.
.SH PLIKI
.BR /etc/rc.local
.BR /etc/rc.serial
.SH "ZOBACZ TAK¯E"
...
Ta wersja zosta³a w miêdzyczasie przerobiona przez Theodore Ts'o
(tytso@mit.edu) 1/1/93. Wszelkie b³êdy i problemy podlegaj± jego
odpowiedzialno¶ci.
